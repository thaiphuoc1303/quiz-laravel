...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <p class="h6">Kết quả: {{isset($bailam) ? $bailam->diem : 'Chưa làm'}} {{isset($bailam) ? '/' : ''}} {{isset($bailam) ? sizeof($checked) : ''}}</p>
        </div>
        @php
            $i = 1;
        @endphp
        @foreach ($baitaps as $item)
            <div class="card cardbaitap">
                ...
                <div><small style="background-color: {{isset($checked[$i - 1]) && $item->Da==$checked[$i - 1] ? '#0ccb1b7a' : '#cb0c0c7a'}} ">Đáp án {{$item->Da== 1 ? 'A' : ''}} {{$item->Da== 2 ? 'B' : ''}}{{$item->Da== 3 ? 'C' : ''}}{{$item->Da== 4 ? 'D' : ''}}</small></div>
            </div>
            @php
                $i++;
            @endphp
